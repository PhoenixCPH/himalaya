...
impl EnvelopeListCommand {
    pub fn execute(
        ...
        let envelopes = Envelopes {
            preset: client.table_preset.clone(),
            arrangement: client.table_arrangement.clone(),
            datetime_fmt: client.datetime_fmt.clone(),
            datetime_local_tz: client.datetime_local_tz,
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        table
            ...
            .add_rows(self.envelopes.iter().map(|env| {
                ...
                row.add_cell(Cell::new(format_size(env.size)));
                row
            }));
...
/// Renders a byte count as a short human-readable string (`12.3K`,
/// `4.5M`, …). Plain decimal-with-1024-base — nothing fancy, just
/// enough to keep the SIZE column compact.
