...
use clap::Parser;
use io_email::coroutines::flag_delete::{FlagDelete, FlagDeleteArg, FlagDeleteResult};
...
fn drive_jmap(
    ...
    for id in ids {
        for flag in flags {
            args.unset_keyword(id.clone(), flag.jmap());
        }
    }

    let inner = JmapEmailSet::new(&jmap.session, &jmap.http_auth, args)?;
    let mut coroutine = FlagDelete::new(inner);
    let mut buf = [0u8; READ_BUFFER_SIZE];
    let mut arg: Option<FlagDeleteArg<'_>> = None;

    loop {
        match coroutine.resume(arg.take()) {
            FlagDeleteResult::Ok => return Ok(()),
            FlagDeleteResult::WantsBytesRead => {
                let n = jmap.stream.read(&mut buf)?;
                arg = Some(FlagDeleteArg::Bytes(&buf[..n]));
            }
            FlagDeleteResult::WantsBytesWrite(bytes) => {
...
fn drive_maildir(
    ...
    for id in ids {
        let inner = MaildirFlagsRemove::new(maildir.clone(), id.as_str(), maildir_flags.clone());
        let mut coroutine = FlagDelete::new(inner);
        let mut arg: Option<FlagDeleteArg<'_>> = None;
